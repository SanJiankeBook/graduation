...
<script type="text/javascript"
	...
	$(function(){
		...
		var aid=${author[0].aid};
 		datagridObj=$('#type_showAuthor_info1').datagrid({
			url:'AuthorNovel',   
			queryParams: {aid:aid},
			fit:true,
			fitColumns:true,
			...
			columns:[[	
						...
						{field : 'npicture',title : '小说封面',width : 20,align : 'center',hidden:false,formatter : function(value, row, index) {
							var picStr = "";
							if (value.indexOf(",")) {
								value = value.split(",");
	...
	function editauthorjsp(){
			var row = $('#type_showAuthor_info1').datagrid('getSelected');
			
		    /* var rows = $('#type_showAuthor_info1').datagrid('getSelections'); */
		    if (row){
		    	if(row.nid!=null){
...
<body>
	<div id="wrapper">
		...
		<table id="type_showAuthor_info1"  data-options="fit:true" ></table>
