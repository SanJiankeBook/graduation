...
changeTheme = function(themeName) {
	...
	var href = '/Examination2.0/jslib/themes/' + themeName + '/easyui.css';
...
function ReLogin(){
	//显示重新登录确认框
	$('#reLoginInfo').window('open');
}
function aboutUs(){
	//显示关于我们的dialog
	$('#aboutUsInfo').window('open');
}
function logoutFun() {
	//显示退出系统确认框
	$('#logoutInfo').window('open');
}
function userInfoFun() {
	$.post("/Examination2.0/login_isLogin.action",function(data){
		$.each(data,function(i,item){
			...
			var examClass=item[0].examClass;
			if(flag=="true"){
				$("#updateExamClass").val(examClass);
