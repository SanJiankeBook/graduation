...
<style>
#showClassSatis_showTable table {
	float: left;
	margin-top: 50px;
	margin-left: 120px;
	margin-top: 10px;
...
#showClassSatis_showTable table tr {
	border: 1px solid #000000;
	width: 80px;
}

#showClassSatis_showTable table td {
	border: 1px solid #000000;
	width: 100px;
	height: 20px;
	font-size: 13px;
...
#showClassSatis_showTable table td p {
	text-align: center;
}
</style>
...
<body>
	...
	<div>
		班级名：<select id="showClassSatis_className">
			<option value="-1" selected="selected">--请选择--</option>
		</select>&nbsp;&nbsp; 年份：<yc:yearSelect id="showClassSatis_year" ></yc:yearSelect> 
		 月份：<yc:monthSelect id="showClassSatis_month"></yc:monthSelect>
		<input type="button" id="showClassSatis_show"
			onClick="showClassSatisfaction()" value="查看">
	</div>

	<div id="showClassSatis_showTable">
		<table width="80%" id="classSatisShow" cellpadding='0' cellspacing='0'>
			<tr>
				<td>老师名</td>
				<td>班级名</td>
				<td>开启时间</td>
	...
	<div id="show_satisfactiondetail" align="center"  style="display: none;width: 90%;margin:0 auto;">
		<div id="class_table_left">
			<table>
				<tr>
					<td >平均分: </td>
					<td id="class_count" style="font-size: 22px;"></td>
				</tr>
				<tr>
					<td>不满意：</td>
...
<script type="text/javascript">
	$(function() {
		...
		$("#showClassSatis_year").val(startyear);
			...
			$.post("findClass.action", function(data) {
				if (data.responseCode == 1) {
					...
					for (var i = 0; i < data.obj.length; i++) {
						$("#showClassSatis_className").append(
								"<option value="+data.obj[i].id+">"
										+ data.obj[i].className + "</option>");
					}
				}
			});
		});
	...
	function showClassSatisfaction() {
		$("#show_satisfactiondetail").css("display","none");
		var className = $("#showClassSatis_className").find("option:selected")
				.text();
		;
		var year = $("#showClassSatis_year").val();
		var month = $("#showClassSatis_month").val();
		if (className == -1) {
			alert("请选择班级！");
			return;
		}
		$
				.post(
						"showClassSatisfactionAction.action",
						{
							year : year,
							month : month,
							className : className
						...
						function(data) {
							$("#classSatisShow  tr:not(:first)").empty("");
							...
							if (data.responseCode == 1) {
								...
								for (var i = 0; i < data.obj.length; i++) {
									...
									$("#classSatisShow")
											.append(
													"<tr><td>"
															+ data.obj[i].teacherName
															+ "</td><td>"
															+ data.obj[i].className
															+ "</td><td>"
															+ data.obj[i].startTime
															+ "</td><td>"
															+ data.obj[i].teacherid
															+ "</td><td>"
															+ data.obj[i].openPersonId
															+ "</td><td>"
															+ data.obj[i].summaryGrade
															+ "</td><td><a href='javascript:find_classdetail("
															+ data.obj[i].id
															+ ",\""
															+ data.obj[i].className
															+ "\")'>详情</a></td></tr>");
	...
	function find_classdetail(id, classname) {
		$.post("showClassSatisfactionDetailAction.action", {
			...
			$("#show_satisfactiondetail").css("display","block")
			//a1-a26 26项评分细则的打分
			var a1 = 0;
			var a2 = 0;
			var a3 = 0;
			...
			$("#class_count").text((count).toFixed(1));
			$("#satis").text(satis);
