...
async def create_pool(loop, **kw):
    ...
    __pool = await aiomysql.create_pool(#创建线程并放进——pool常量里面
        host='127.0.0.1', #主机ip，默认本机
        port=3306, #端口，默认3306
        user='root', #用户
        password='root', #用户口令
        db='test', #选择数据库
        charset='utf8', #设置数据库编码，默认utf8
        autocommit= True, #设置自动提交事务，默认打开
        maxsize=10, #设置最大连接数，默认10
        minsize= 1, #设置最少连接数，默认1
...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs): #用metaclass=ModelMetaclass创建类时，通过这个方法生成类
        ...
        logger.info('found model: %s (table: %s)' % (name, tableName)) #类名、表名
...
# 创建一个实例
    ...
    u = User(id=1238, name="ReedSun", email="sunhongzhao@foxmail.com", password="nicaicai")
    loop = asyncio.get_event_loop()
#     task=[ create_pool(loop),u.save()]
